...
./venv/bin/python -m src.api.main > logs/api.log 2>&1 &
API_PID=$!
...
./venv/bin/python scripts/run_autonomous_hunter.py > logs/hunter.log 2>&1 &
HUNTER_PID=$!
echo "   -> Hunter active (PID: $HUNTER_PID)"

# 4. تشغيل الواجهة (Dashboard)
echo -e "${GREEN}🖥️  Launching Command Center...${NC}"
./venv/bin/streamlit run src/interface/dashboard.py --server.port 8501 --theme.base "dark" &
DASHBOARD_PID=$!
