...
# -*- coding: utf-8 -*-
"""
Knowledge Indexer - Implementation for knowledge indexing
"""
...
# This is the main implementation now.
# The dummy file in `core` has been removed.
...
class KnowledgeIndexer:
    def __init__(self):
        ...
        self.metadata = {
            ...
            "fallback": False # No longer a fallback
        }
    ...
    def load_index(self, filepath=None):
        """Load index from file (in-memory does nothing)"""
        warnings.warn("In-memory KnowledgeIndexer does not support loading from file.")
        return {"status": "loaded", "fallback": False, "entries": len(self.entries)}
    
    def save_index(self, filepath=None):
        """Save index to file (in-memory does nothing)"""
        warnings.warn("In-memory KnowledgeIndexer does not support saving to file.")
        return {"status": "saved", "fallback": False, "entries": len(self.entries)}
